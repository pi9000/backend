    ...
    <table id="noPaginationTable"
        ...
        <tbody style="text-align:center;color:black">
            ...
            <tr>
                ...
                <td>
                    <span id="suspend_{{ $item->extplayer }}_display">{{ $item->status_game == 1 ? 'On' : 'Off'}}</span>
                ...
                <td>
                    <span id="verified_{{ $item->extplayer }}_display">{{ $item->verified == 1 ? 'Yes' : 'No'
                        }}</span>
                ...
                <td class="text-middle"> {{ $item->created_at ? date('Y-m-d H:i:s', strtotime($item->created_at)) : ''
                    }} </td>
                <td class="text-middle"> {{ $item->first_deposit_date ? date('Y-m-d H:i:s',
                    strtotime($item->first_deposit_date)) : '' }} </td>
                <td class="text-middle"> {{ $item->last_deposit_date ? date('Y-m-d H:i:s',
                    strtotime($item->last_deposit_date)) : '' }} </td>
                <td class="text-middle"> {{ $item->last_login_time ? date('Y-m-d H:i:s',
                    strtotime($item->last_login_time)) : '' }} </td>
                ...
                <td>
                    <a href="javascript:void(0)" class="waves-effect waves-light btn btn-warning btn-sm btn-rounded"
                        onclick="edit_button(this, `{{ $item->extplayer }}` )"> &#9998; </a>
...
<script>
    ...
    function edit_button(e, id) {
        $(`#suspend_${id} , #status_${id} , #firstname_${id} , #lastname_${id} , #lock_${id}, #mobile_${id} , #old_mobile_${id} , #password_${id}, #verified_${id}`).show();
